...
from query_handler import LambdaQueryHandler

app = Flask(__name__)
query_handler = LambdaQueryHandler()
CORS(app)

@app.route('/users', methods=['GET'])
def users():
    response = query_handler.call("neo4j_query_handler_function", request.data)
    return jsonify(response)

@app.route('/user/<string:user_id>', methods=['GET'])
def user(user_id):
    payload = {"user_name": user_id}
    response = query_handler.call("neo4j_query_handler_function", payload)
    return jsonify(response)
